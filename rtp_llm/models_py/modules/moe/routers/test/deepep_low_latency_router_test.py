...
from rtp_llm.distribute.worker_info import g_parallel_info, update_master_info, g_master_info
...
def _init_router(rank: int, use_fp8: bool):
    # set env
    os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(str(i) for i in range(int(os.environ["WORLD_SIZE"])))
...
def _run_deepep_low_latency_router_test(rank: int, use_fp8: bool):
    ...
    ll_num_token_per_rank = (config.max_generate_batch_size + config.tp_size - 1) // config.tp_size
    num_max_tokens = ll_num_token_per_rank * ep_size

    # for per dp rank
    num_token_per_rank = config.max_generate_batch_size
    int_mask = (2**32) - 1
    hidden_states = torch.randn((dp_size, num_token_per_rank, hidden_size)).to(torch.bfloat16)
    ...
    topk_ids = torch.rand(dp_size, num_token_per_rank, num_experts).topk(num_topk, dim=-1, largest=True)[1]
    topk_weights = (torch.ones((dp_size, num_token_per_rank, num_topk)).to(torch.float32) / num_topk)
