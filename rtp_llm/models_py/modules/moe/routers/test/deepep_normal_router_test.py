...
def worker_function(rank: int, use_fp8: bool, token_num_per_rank: List[int]):
    ...
    try:
        ...
        for i in range(5):
            ...
            a2 = router.finalize(combine_x, topk_weights, topk_ids, False, None, extra_finalize_args)
            if router.use_fp8:
...
def test_single(world_size: int, test_tp_size: int, use_fp8: bool):
    with PortsContext(None, 1) as ports:
        ...
        # 启动world_size个进程
        processes = []
        token_num_per_rank = [random.randint(4, 12) // 4 * 4 for _ in range(world_size // test_tp_size)]
