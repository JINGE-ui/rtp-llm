    ...
    def finalize(
        ...
        if tp_size > 1:
            ...
            if combined_x.size(0) < tp_token_size:
                padding_combined_x = torch.empty(size = (tp_token_size - combined_x.size(0), combined_x.size(1)),
                    device=combined_x.device, dtype=combined_x.dtype)
                combined_x = torch.cat([combined_x, padding_combined_x], dim=0)

            gatherd_output = all_gather(combined_x, group=Group.TP).reshape(tp_size * tp_token_size, -1)
