...
class DeepepNormalRouter(FusedMoeDataRouter):
    ...
    def prepare(
        ...
        (
            ...
            is_token_in_rank,
            event1,
        ) = self.deepep_buffer_wrapper.buffer.get_dispatch_layout(tp_expert_ids, num_experts)
        # dispatch
        (
            output,
            recv_topk_idx,
    ...
    def finalize(
        ...
        if tp_size > 1:
            ...
            if out_token.size(0) < tp_token_size:
                padding_out_token = torch.empty(size = (tp_token_size - out_token.size(0), out_token.size(1)),
                    device=out_token.device, dtype=out_token.dtype)
